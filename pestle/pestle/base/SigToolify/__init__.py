...
class SigToolify:
    ...
    def new(self):
        ...
        template_path = os.path.join(
            pestle_path, "pestle/base/SigToolify/templates/SigToolTemplate/"
        )
        print(template_path)
        ...
        extra_context = {"release_date": date, "pestlepath": pestle_path}
    ...
    def convert_toolname_to_sigName(self, sigName):
        return "tooSig{}".format(sigName.split("_")[1].capitalize())
    ...
    def tool_files(self):
        ...
        print(
            "Tool Location:\n {}".format(
                registry.loc[registry["toolname"] == toolname, "tool_location"]
                .item()
                .replace("PESTLE_PATH/", pestle_path)
            )
        )
        mod_path = (
            registry.loc[registry["toolname"] == toolname, "module_path"]
            .item()
            .replace("PESTLE_PATH/", pestle_path)
        )
        print("Module path:\n {}".format(mod_path))
        ...
        print(
            "Arguments path:\n {}".format(
                registry.loc[registry["toolname"] == toolname, "args_file"]
                .item()
                .replace("PESTLE_PATH/", pestle_path)
            )
        )
        print(
            "Tests path:\n {}".format(
                registry.loc[registry["toolname"] == toolname, "test_file"]
                .item()
                .replace("PESTLE_PATH/", pestle_path)
            )
        )

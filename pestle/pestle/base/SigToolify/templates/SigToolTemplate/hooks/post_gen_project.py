...
tool_dest = os.path.join(pestlepath, "tools", "{{cookiecutter.toolname}}.py")
os.rename(tool_src, tool_dest)

# Move tests
tests_src = os.path.join(
    pestlepath,
    "pestle",
    "sigtools",
    "{{ cookiecutter.SigToolname }}",
...
tests_dest = os.path.join(pestlepath, "tests", "test{{cookiecutter.SigToolname}}.py")
...
tool_dir = os.path.join(
    "PESTLE_PATH", "pestle", "sigtools", "{{ cookiecutter.SigToolname }}/"
)
new_tool["sigName"] = ["{{ cookiecutter.SigToolname }}"]
new_tool["toolname"] = ["{{ cookiecutter.toolname }}"]
new_tool["tool_location"] = [
    os.path.join("PESTLE_PATH", "tools", "{{cookiecutter.toolname}}.py")
]
...
new_tool["test_file"] = [
    os.path.join("PESTLE_PATH", "tests", "test{{cookiecutter.SigToolname}}.py")
]
new_tool = pd.DataFrame(data=new_tool)

if os.path.exists(registry_file):
    registry = pd.read_csv(registry_file, sep="\t")
    registry = registry.append(new_tool).sort_values(by=["toolname"])
